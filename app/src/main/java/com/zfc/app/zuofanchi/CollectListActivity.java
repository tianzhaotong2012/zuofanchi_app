...
public class CollectListActivity extends Activity{
    ...
    void initData(){
        List<PostItem> collectList = getCollectedPosts();
        if(collectList.size() == 0){
            Toast.makeText(getBaseContext(), "还没有添加任何收藏", Toast.LENGTH_SHORT).show();
        }
        for (int i = 0; i < collectList.size(); i++) {
            DiseaseBankEntity bank = new DiseaseBankEntity();
            bank.setId(String.valueOf(i));
            PostItem postItem = collectList.get(i);
